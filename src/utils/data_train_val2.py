...
#        global_t_preds, global_t_labels = self.all_gather_lab_preds(self.t_preds, self.t_labels)
    ...
    def valid_epoch(self, epoch, val_proteindataloader):
        ...
        #g_v_labels = [id.item() for id in global_v_labels]
        #g_v_preds  = [id.item() for id in global_v_preds ]
        #l_v_labels = [id.item() for id in self.v_labels]
        #l_v_preds  = [id.item() for id in self.v_preds]
...
#        print("DEBUG", len(labels), len(preds), len(protein_dataloader.df))
...
#        self.initialize_files()
        ...
        for epoch in range(self.epochs_run, self.max_epochs):
            ...
            #g_t_mse  = torch.mean(         (torch.tensor(g_t_labels) - torch.tensor(g_t_preds))**2)
            #g_t_mae  = torch.mean(torch.abs(torch.tensor(g_t_labels) - torch.tensor(g_t_preds))   )
            #g_t_rmse = torch.sqrt(g_t_mse)
            #g_t_corr, _ = pearsonr(g_t_labels, g_t_preds)
            #l_t_mse  = torch.mean(         (torch.tensor(l_t_labels) - torch.tensor(l_t_preds))**2)
            #l_t_mae  = torch.mean(torch.abs(torch.tensor(l_t_labels) - torch.tensor(l_t_preds))   )
            #l_t_rmse = torch.sqrt(l_t_mse)
            #l_t_corr, _ = pearsonr(l_t_labels, l_t_preds)
            ...
            for val_no, val_dl in enumerate(self.val_dls):
                ...
                #g_v_mse  = torch.mean(         (torch.tensor(g_v_labels) - torch.tensor(g_v_preds))**2)
                #g_v_mae  = torch.mean(torch.abs(torch.tensor(g_v_labels) - torch.tensor(g_v_preds))   )
                #g_v_rmse = torch.sqrt(g_v_mse)
                #g_v_corr, _ = pearsonr(g_v_labels, g_v_preds)
                #l_v_mse  = torch.mean(         (torch.tensor(l_v_labels) - torch.tensor(l_v_preds))**2)
                #l_v_mae  = torch.mean(torch.abs(torch.tensor(l_v_labels) - torch.tensor(l_v_preds))   )
                #l_v_rmse = torch.sqrt(l_v_mse)
                #l_v_corr, _ = pearsonr(l_v_labels, l_v_preds)
